...
void Sprite::Update(float dt) {
    if(secondsToSelfDestruct > 0){
        selfDestructCout.Update(dt);

        if(selfDestructCout.Get() >= secondsToSelfDestruct){
...
Sprite::Sprite(GameObject &associated, string file, int frameCount, float frameTime,
               float secondsToSelfDestruct) : Component
                                                      (associated), texture(nullptr), width(0), height(0),
                                              scale(1, 1), frameCount(frameCount), frameTime
                                                      (frameTime), currentFrame(0), timeElapsed(0),
                                              secondsToSelfDestruct(secondsToSelfDestruct),
                                              selfDestructCout(*new Timer()) 
{
    Open(file);
}
